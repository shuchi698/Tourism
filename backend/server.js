...
mongoose.connect(dbConfig.db, {
 ...
 app.post('/create', (req, res) => {
    ...
    form.on('fileBegin', (name, file) => {
       file.path = path.join(__dirname, '..', 'tourism-management-system', 'src', 'assets', 'attractions', file.name);
    ...
    form.parse(req, (err, fields, file) => {
       ...
       MongoClient.connect(url, function(err, db) {
            if (err) throw err;
               var dbo = db.db("tourism-management-system");
...
app.use('/users', usersRouter);
 ...
 //simple route
app.get("/",(req,res)=>{
   res.json({message:"Welcome to tourism management system"});
});
 //PORT USAGE
